...
export default function PricingCard({ plan, index }) {
  ...
  return (
    <article
      className={[
        'relative flex flex-col w-full h-full border rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.03)] hover:shadow-[0_16px_40px_rgba(0,0,0,0.07)] hover:border-black/25 transition-all duration-300',
    ...
    >
      ...
      {plan.popular && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 z-10">
          <div className="inline-flex items-center gap-1.5 text-[11px] font-heading font-semibold text-white bg-black px-4 py-1 rounded-xl shadow-md">
      ...
      <div className="p-6 sm:p-7 flex flex-col justify-between gap-5 flex-1">
            ...
            {plan.features.map((feature) => (
              <li key={feature.label} className="flex items-start gap-2.5">
                {feature.included ? (
                  ...
                  <div className="w-4 h-4 rounded-full bg-red-700 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <X size={9} className="text-white" strokeWidth={2} />
                  </div>
                )}
                <span
                  className={[
        ...
        {/* Full-Page Checkout Action Link */}
        <Link
          to={`/checkout?plan=${plan.id}`}
          id={`pricing-cta-${plan.id}`}
          className="btn-micro1 w-full justify-between shadow-sm"
        >
